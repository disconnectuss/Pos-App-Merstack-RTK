...
const sunburst_1 = require("../mark/sunburst");
// Get sunburst element.
const getElementsSunburst = (plot) => {
    return plot
        .querySelectorAll('.element')
        .filter((item) => (0, util_1.get)(item, ['style', sunburst_1.SUNBURST_TYPE_FIELD]) === sunburst_1.SUNBURST_TYPE);
};
function selectPlotArea(root) {
    return (0, g2_1.select)(root).select(`.${g2_1.PLOT_CLASS_NAME}`).node();
}
...
function DrillDown(drillDownOptions = {}) {
    ...
    return (context) => {
        ...
        const sunburstMark = options.marks.find(({ id }) => id === sunburst_1.SUNBURST_TYPE);
        ...
        const drillDownClick = (path, depth) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (path) {
                ...
                [rootText, ...drillTexts].forEach((item, index) => {
                    ...
                    item.addEventListener('click', () => {
                        drillDownClick(item.name, (0, util_1.get)(item, ['style', 'depth']));
                    });
                });
            }
            // Update marks.
            setState('drillDown', (viewOptions) => {
                ...
                const newMarks = marks.map((mark) => {
                    if (mark.id !== sunburst_1.SUNBURST_TYPE && mark.type !== 'rect')
                        return mark;
                    ...
                    const newData = data.filter((item) => {
                        ...
                        if (fixedColor) {
                            item[sunburst_1.SUNBURST_ANCESTOR_FIELD] = key.split(' / ')[depth];
                        }
                        if (!path)
                            return true;
                    ...
                    return (0, util_1.deepMix)({}, mark, fixedColor
                        ? {
                            data: newData,
                        }
                        : {
        ...
        const createDrillClick = (e) => {
            ...
            if ((0, util_1.get)(item, ['style', sunburst_1.SUNBURST_TYPE_FIELD]) !== sunburst_1.SUNBURST_TYPE ||
                (0, util_1.get)(item, ['markType']) !== 'rect' ||
                !(0, util_1.get)(item, ['style', partition_1.CHILD_NODE_COUNT]))
                return;
            const path = (0, util_1.get)(item, ['__data__', 'key']);
            const depth = (0, util_1.get)(item, ['style', 'depth']);
